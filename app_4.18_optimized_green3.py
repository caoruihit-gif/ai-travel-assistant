...
DEFAULT_THEME = {
    "hero": "https://images.unsplash.com/photo-1500530855697-b586d89ba3ee?auto=format&fit=crop&w=1600&q=80",
    "page": "linear-gradient(180deg, rgba(248,251,253,0.97), rgba(248,250,252,0.99)), radial-gradient(circle at top right, rgba(20,184,166,0.10), transparent 25%), radial-gradient(circle at top left, rgba(59,130,246,0.08), transparent 22%)",
    "hint": "答案都在路上，自由都在风里。",
    "primary": "#0f766e",
    "secondary": "#14b8a6",
    "accent": "#f59e0b",
...
def show_generation_feedback() -> tuple:
    ...
    messages = json.dumps(LOADING_MESSAGES, ensure_ascii=False)
    components.html(
        f"""
        <div style="background:linear-gradient(135deg, rgba(255,255,255,0.98), rgba(248,250,252,0.96)); border:1px solid rgba(148,163,184,0.18); border-radius:24px; padding:22px 20px; box-shadow:0 14px 30px rgba(15,23,42,0.06); margin-bottom:14px; overflow:hidden; position:relative;">
            <div style="display:flex; align-items:center; gap:16px;">
                <div id="travel-illustration" style="width:92px; height:92px; flex:0 0 92px;"></div>
                <div style="flex:1; min-width:0;">
                    <div style="font-size:1.02rem; font-weight:800; color:#0f172a; margin-bottom:6px;">旅途计划生成中</div>
                    <div id="travel-msg" style="font-size:0.96rem; color:#475569; line-height:1.75; min-height:54px;"></div>
                    <div style="margin-top:12px; height:8px; background:rgba(191,219,254,0.45); border-radius:999px; overflow:hidden;">
                        <div id="travel-bar" style="height:100%; width:12%; background:linear-gradient(90deg, #60a5fa, #818cf8); border-radius:999px; transition:width .45s ease;"></div>
                    </div>
                </div>
            </div>
        </div>
        <script>
            const msgs = {messages};
            const el = document.getElementById('travel-msg');
            const bar = document.getElementById('travel-bar');
            const art = document.getElementById('travel-illustration');
            const svgs = [
              `<svg viewBox="0 0 120 120" width="92" height="92" xmlns="http://www.w3.org/2000/svg"><rect x="8" y="18" width="104" height="78" rx="20" fill="#eff6ff"/><path d="M28 74c9-17 17-25 23-25 8 0 11 9 17 9 6 0 9-11 18-11 8 0 14 10 24 27" fill="none" stroke="#60a5fa" stroke-width="7" stroke-linecap="round" stroke-linejoin="round"/><circle cx="43" cy="40" r="7" fill="#fbbf24"/></svg>`,
              `<svg viewBox="0 0 120 120" width="92" height="92" xmlns="http://www.w3.org/2000/svg"><rect x="12" y="18" width="96" height="84" rx="22" fill="#f8fafc"/><path d="M28 80h64" stroke="#94a3b8" stroke-width="6" stroke-linecap="round"/><path d="M37 80V50l23-13 23 13v30" fill="#dbeafe" stroke="#60a5fa" stroke-width="5" stroke-linejoin="round"/><path d="M60 37V26" stroke="#818cf8" stroke-width="6" stroke-linecap="round"/></svg>`,
              `<svg viewBox="0 0 120 120" width="92" height="92" xmlns="http://www.w3.org/2000/svg"><circle cx="60" cy="60" r="44" fill="#eef2ff"/><path d="M36 72c9-17 16-28 24-28s12 10 20 10 12-8 20-8" fill="none" stroke="#818cf8" stroke-width="7" stroke-linecap="round" stroke-linejoin="round"/><path d="M35 36h50" stroke="#f59e0b" stroke-width="7" stroke-linecap="round"/></svg>`
            ];
            let i = 0;
            let p = 12;
            function tick() {{
                el.innerText = msgs[i % msgs.length];
                art.innerHTML = svgs[i % svgs.length];
                p = Math.min(92, p + 10);
                bar.style.width = p + '%';
                i += 1;
            }}
...
def finish_generation_feedback(box, progress, success: bool = True, message: str = "") -> None:
    progress.progress(100 if success else 0)
    box.markdown(
        f"""
        <div class="loading-box">
            <div class="loading-title">{'旅途已经有模样了 ✨' if success else '这次没能顺利排好行程'}</div>
    ...
    st.markdown(
        ...
        <div class="hero">
            ...
            <div class="hero-badges">
                <div class="hero-badge">快乐不必等 完美时机，我们出发就现在！</div>
...
def render_history_card() -> None:
    ...
    st.markdown(
        ...
        <div class="glass-card">
            ...
            <div class="section-title">你最近让我们一起构思过这些地方</div>
        """,
        unsafe_allow_html=True,
    )
    for item in history:
...
def render_overview_tab(plan: Dict[str, Any], theme: Dict[str, str]) -> None:
    ...
    render_section_copy(section_overview_text(plan), "copy_overview_section")
...
def render_timeline_tab(plan: Dict[str, Any], theme: Dict[str, str]) -> None:
    ...
    render_section_copy(section_timeline_text(plan), "copy_timeline_section")
...
def render_food_tab(plan: Dict[str, Any], theme: Dict[str, str]) -> None:
    ...
    render_section_copy(section_food_text(plan), "copy_food_section")
...
def render_hotel_tab(plan: Dict[str, Any], theme: Dict[str, str]) -> None:
    ...
    render_section_copy(section_hotel_text(plan), "copy_hotel_section")
...
def render_budget_packing_tab(plan: Dict[str, Any]) -> None:
    ...
    render_section_copy(section_budget_text(plan), "copy_budget_section")
    ...
    with right:
        ...
        if input_values["generate"]:
            if not destination:
                ...
                box, progress = show_generation_feedback()
                try:
                    ...
                    finish_generation_feedback(box, progress, True, "旅途已经排出轮廓了，往下滑看看细节吧。")
                except Exception as e:
                    st.session_state["plan_error"] = str(e)
                    st.session_state.pop("plan_json", None)
                    st.session_state.pop("plan_markdown", None)
                    finish_generation_feedback(box, progress, False, f"这次没能顺利排好：{e}")
        ...
        if "plan_json" not in st.session_state:
            st.markdown(
                ...
                <div class="friendly-empty">
                    ...
                    <div class="section-title">先从左边把你的想法写下来吧</div>
